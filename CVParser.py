...
class CVParser():
    ...
    def standardizeCVDict(self, cv_dict, remove_duplicates=True):
        ...
        for key in standardized_cv_dict:
            value = standardized_cv_dict.get(key, None)  # Safely get value, default to None
            if value is None:
                continue  # Skip processing if value is missing or None
        
            if isinstance(value, list):
                for i in range(len(value)):
                    if isinstance(value[i], dict):
                        value[i] = dictToText(value[i])
                    
                if remove_duplicates:
                    standardized_cv_dict[key] = removeDuplicates(list_data=value)
                
            elif isinstance(value, dict):
                standardized_cv_dict[key] = dictToText(value)
